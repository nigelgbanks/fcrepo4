...
public class FedoraNodes extends AbstractResource {
    ...
    @DefaultValue("")
    final String label, @QueryParam("mixin")
    ...
    final MediaType requestContentType, final InputStream requestBodyStream)
        ...
        return createObject(pathList, label, mixin, checksumType, checksum,
                requestContentType, session, uriInfo, requestBodyStream);
    }

    /**
     * Creates a new object with explicit context.
     * 
     * @param pid
     * @return 201
     * @throws RepositoryException
    ...
    @DefaultValue("")
    final String label, @QueryParam("mixin")
    @DefaultValue(FEDORA_OBJECT)
    final String mixin, @QueryParam("checksumType")
    final String checksumType, @QueryParam("checksum")
    final String checksum, @HeaderParam("Content-Type")
    final MediaType requestContentType, @InjectedSession
    final Session session, @Context
    final UriInfo uriInfo, final InputStream requestBodyStream)
        ...
        try {
            ...
            if (FedoraJcrTypes.FEDORA_DATASTREAM.equals(mixin)) {
                ...
                final Datastream ds = new Datastream(result);
                ds.setLabel(label);
            }
            session.save();
